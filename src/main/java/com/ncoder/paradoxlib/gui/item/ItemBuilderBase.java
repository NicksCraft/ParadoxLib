package com.ncoder.paradoxlib.gui.item;
...
import com.ncoder.paradoxlib.enums.ColorType;

/**
 * A helper for creating and modifying {@link ItemStack}s.
 *
 * <p>
 * <b>NOTE:</b> in future, this class will be renamed to ItemBuilder (see
 * {@link ItemBuilder}). For now, you should
 * continue to use {@link ItemBuilder} as it is intended to be a seamless
 * replacement.
 *
 * <p>
 * This class provides a convenient chainable ('builder pattern') API for
 * manipulating the metadata of an
 * {@link ItemStack}, replacing several calls into a clean one-liner in many
 * cases.
 *
 * <p>
 * <b>Note:</b> as a convention (to make debugging and identifying potential
 * issues simpler) all methods on this
 * interface shall throw a {@link NullPointerException} if the item meta is
 * null. This would only happen if Bukkit's API
 * failed to produce an {@link ItemMeta} object for an item type - which
...
public interface ItemBuilderBase {
    ...
    /**
     * Create a new {@link ItemBuilderBase} that uses the provided {@link ItemStack}
     * and associated metadata as the
     * initial configuration.
     *
     * @throws IllegalArgumentException if the item stack's type (material) is a
     *                                  non-item type.
     ...
     *
     * <p>
     * Color codes using the ampersand ({@code &}) are translated, if you want to
     * avoid this, you should wrap your
    ...
    /**
     * Returns either the display name of the item, if it exists, or null if it
     * doesn't.
     *
     * <p>
     * You should note that this method fetches the name directly from the stack's
     * {@link ItemMeta}, so you should
     * take extra care when comparing names with color codes - particularly if you
     * used the {@link #name(String)} method
     * as they will be in their translated sectional symbol (§) form, rather than
     * their 'coded' form ({@code &}).
     *
     * <p>
     * For example, if you used {@link #name(String)} to set the name to '&amp;cMy
     * Item', the output of this method
     * would be '§cMy Item'
     *
     ...
     *
     * @param lore The desired lore of the item, with each line as a separate
     *             string.
     * @return The {@link ItemBuilderBase} instance.
     */
    @Nonnull
    ...
    /**
     * Sets the lore of the item. As with {@link #name(String)}, color codes will be
     * replaced. Each string represents a
     * line of the lore.
     *
     * <p>
     * Lines will not be automatically wrapped or truncated, so it is recommended
    ...
    /**
     * Gets the lore of the item as a list of strings. Each string represents a line
     * of the item's lore in-game.
     *
     * <p>
     * As with {@link #name(String)}, it should be noted that color-coded lore lines
     * will be returned with the colors
     ...
     *
     * <p>
     * The behavior of this method is undefined when an item does not have color
     ...
     */
    @Nonnull
    ItemBuilderBase color(@Nonnull ColorType color);
     ...
     * <p>
     * The behavior of this method is undefined when an item does not have color
     * values associated with it and may be
     * inconsistent across versions of the game.
     *
     * @return The {@link ColorType} of the item (where the item has a color
     *         associated).
     */
    @Nullable
    ColorType getColor();
     ...
     *
     * <p>
     * The behavior of this method is undefined when an item does not have a data
     * value associated with it.
     *
     * <p>
     * In newer versions of the game (Minecraft 1.13+), this method throws
     * {@link UnsupportedOperationException} to
     * encourage use of a different method (or to open a GitHub issue if
     * functionality has been missed).
     *
     ...
     *
     * <p>
     * Does nothing if the maximum durability cannot be set on the item.
     *
     ...
     *
     * <p>
     * Returns zero if the item does not have a max damage value.
     *
     ...
     *
     * <p>
     * This method uses {@link ItemStack#addUnsafeEnchantment(Enchantment, int)}
     * rather than
     * {@link ItemStack#addEnchantment(Enchantment, int)} to avoid the associated
     * checks of whether level is within the
    ...
    /**
     * If the item has {@link SkullMeta} (i.e. if the item is a skull), this can be
     * used to set the skull's owner (i.e.
     * the player the skull represents.)
     *
     * <p>
     * In older versions of the game, this also sets the skull's data value to 3 for
     * 'player head', as setting the
     * skull's owner doesn't make much sense for the mob skulls. (This is irrelevant
     * in later versions as the skull
     ...
     *
     * <p>
     * This can be useful when applying stream operations to
     * {@link ItemBuilderBase}s.
     *
     * <p>
     * The {@link ItemBuilderBase} instance is supplied to both the predicate
     * (condition) and result function.
     *
     * <p>
     * For example:
     *
     ...
     * }</pre>
     *
     * @param ifTrue The condition upon which, <code>then</code> should be
     *               performed.
     * @param then   The action to perform if the predicate, <code>ifTrue</code>, is
     *               true.
     * @return The {@link ItemBuilderBase} instance.
     */
    @Nonnull
    ...
    /**
     * Returns the {@link ItemStack} that has been configured on the
     * {@link ItemBuilderBase}.
     *
